...
## 5-watch 于 computed的区别

## 6-两种路由模式的区别？

#### 1-hash模式：

通过监听`onhashchange`事件实现路由切换。

##### 优点：

​	1-后端不要去配置了，可以直接使用，可以直接通过url访问对应的页面；

​	2-由于使用h5中的 history api 所以兼容性更好；
...
​	2-不支持h5中 history api 的前进，后退等操作；

#### 2-history模式：

通过`History`对象的api实现：

pushState：**`history.pushState()`** 方法向当前浏览器会话的历史堆栈中添加一个状态（state）。
...
​	1-url中没有#号，更加简洁，同时利于seo，因为会发起更多的网络请求；

​	2-支持使用 h5 中 history 的API；

##### 缺点：

​	1-由于是单页面应用，需要后端配置，任何情况下都返回同一个页面；
