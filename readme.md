...
3. 点击劫持：**一种在网页中将恶意代码等隐藏在看似无害的内容（如按钮）之下，并诱使用户点击的手段**。

   1. 实现过程：下面是一个基本的点击劫持攻击过程：

      1. 攻击者制作一个网页（称为“攻击页面”），并在该网页上嵌入一个含有“目标页面”的iframe，同时将iframe隐藏。
      2. 攻击者通过CSS样式，将iframe的透明度设置为0，从而使iframe变得透明，用户无法察觉该iframe的存在。
      3. 攻击者将“目标页面”上的某个元素，如按钮、超链接等，通过CSS样式将其覆盖在iframe的上层，并且用户看不到覆盖在iframe上的该元素。
      4. 当用户在“目标页面”上点击该元素时，实际上是触发了iframe中的某个元素的点击事件，而不是用户原本预期的操作。
      5. 攻击者可以利用此方式，通过iframe执行一些非法的操作，例如盗取用户的敏感信息、模拟用户的行为等。

   2. 如何防范：

      1. X-Frame-Options 防御策略：在HTTP响应头中添加X-Frame-Options字段，通过该字段控制是否允许iframe加载当前页面，建议设置为DENY【任何网站都不能够嵌套】或SAMEORIGIN【同域名页面可以嵌套】；
      2. JS禁止嵌套页面：在页面中添加JavaScript脚本，通过**window.top**【返回最顶层的窗口对象】对象判断页面是否在顶层窗口打开，从而禁止页面在iframe中嵌套。
...
4. 钓鱼网站攻击：攻击者通过伪造网站界面、仿冒网站、虚假登录页面等，诱骗用户输入敏感信息，如用户名、密码、信用卡号等。
...
#### 	VUE2.x 中数据劫持存在问题：
1.无法监测到 数组的变化：尽管`Object.defineProperty`提供了检测数组变化的能力，但在数组增删场景下的检测和vue本身处理的性能考虑，并没有利用改语法的检测能力
...
1. 静态编译：es6模块化，在编译的时候就可以确定模块之间的关系，使得编译器可以对模块进行优化，减少运行时的开销。而在commonjs中，模块的依赖关系在运行时动态确定的，而且多次引用同一个模块会重复的执行和加载。

2. 重复执行依赖模块：commonjs中，模块的依赖关系在运行时动态确定的，而且多次引用同一个模块会重复的执行和加载。

3. 循环加载：a模块依赖b、b又依赖a的这种循环加载的情况，在commonjs中只会执行已经解析到的代码，而es6模块则能全部处理完，这也归功于es6导入模块只是该模块的引用，而不是执行后拷贝的值

​	**为什么Commonjs重复引用模块会再一次执行，加载？**当一个模块被引用时，CommonJS 模块化会先检查该模块是否已经被加载，如果没有加载过，则会执行该模块的代码，并将导出的内容缓存起来。当其他模块再次引用该模块时，CommonJS 模块化会直接从缓存中取出该模块的导出内容，而不会再次执行该模块的代码。

但是，当一个模块被重复引用时，由于**模块的缓存是按照模块的路径**来管理的，**而不是按照模块的内容**来管理的，所以会导致模块再一次执行和加载。因为重复引用时会使用不同的路径引用同一个模块，导致模块缓存失效，模块会再次被加载和执行。

3.模块拷贝的区别：

​	在 CommonJS 中，每次引用一个模块时，都会重新执行一遍该模块的代码，并且会将模块的导出内容复制一份返回给引用方。因此，每个模块的导出内容在内存中都会存在多个副本，每个副本都有自己的状态，互不干扰。

而在 ES6 模块化中，模块的导出内容只会在**第一次被引用时执行一遍**，并且只会有一份副本被创建和返回，后续的引用会直接使用该副本，而不会重新执行一遍代码或者复制一份。这种方式被称为**单例模式**，即所有引用该模块的代码都引用的是同一个实例，因此可以**共享该模块的状态**。

​	**es6模块化采用单例模式的优点？**

​	1-减少代码的执行次数：当一个模块被多个代码块引用时，只会执行一次该模块的代码，而不是每次都执行。这可以显著降低应用程序的运行时间，提高性能。

​	2-确保模块的导出内容唯一：由于每个模块只有一个实例，因此在应用程序中的任何地方引用该模块时，都会引用同一个实例。这样可以确保模块的导出内容是唯一的，避免了由于多个实例之间状**态不同步而导致的错误**。

4.支持异步加载：ES6 模块支持异步加载，可以提高应用的性能和响应速度。
...
![](https://cdn.becase.xyz/20221001154626.png)
当嵌套层数超过五层即为 `4ms` 
...
​		1-编译代码能力，webpack集成babel等插件后，能够把开发阶段使用的例如像 ES6+ 的语法转译成 ES5 语法，从而提高开发效率，解决浏览器兼容问题；

​		2-模块整合能力：在生产环境的时候，可以把散落的模块打包到一个 bundle.js，提高可维护性，**解决了浏览器频繁请求模块文件的的问题**；

​		3-万物皆模块的能力：webpack 配合loader可以将开发过程中使用的 **样式、图片、字体等资源文件** 都作为模块使用，这样就拥有了一个统一的模块化方案，所有 **资源文件的加载都可以通过代码控制** ，可以与业务代码一同维护，提高项目可维护性；
